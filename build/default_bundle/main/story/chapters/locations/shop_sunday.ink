...
=== shop_drinks_pre_date ===
# speaker:none
Холодильники гудят ровно, будто у них воскресенье никогда не сбивается. За стеклом — вода, холодный чай, газировка, кофе в банке и печенье в прозрачной упаковке.
...
* [Взять печенье]
    # speaker:mc
    Печенье — это универсально. Или ты заботишься, или просто хотел{mc_gender == "female":а|} что-то сладкое.
...
- else:
    Касса стоит у выхода: терминал, сканер, маленькая зона ожидания. {not met_npc_sunday:
        {sunday_gift_bought:
            Подарок уже выбран. Касса будто специально делает вид, что это обычная покупка: короткий писк, тонкий чек, пакет, который почти ничего не весит.

            # speaker:mc
            Теперь главное — не начать выбирать заново от нервов.
        - else:
...
// ================================================================
// В iter 1 это flavor-комната после встречи, а до свидания — место
// выбора одного небанального подарка через Ink-список внутри категорийных
// хотспотов. Не дробить бытовой отдел на отдельные Lua gift-hotspots:
// UX должен совпадать с напитками/снеками — клик по категории, затем
// текстовый выбор и вариант отмены.
//
...
# speaker:npc
У этой полки очень аппетитное печенье.
