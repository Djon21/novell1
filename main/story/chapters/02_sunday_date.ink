...
{npc_name} нигде не видно. Не у лавочки, не у перил, не на дорожке к воде.

# speaker:mc
Ладно. Значит, сначала — не паниковать. Потом — написать.

# speaker:none
Можно осмотреться и написать в Messenger, где {npc_name}.
...
{not park_where_message_sent:
    Можно написать {npc_name} и уточнить, где вы разминулись.
- else:
    {park_npc_greeted:
...
Рядом с {npc_name} эта дорожка перестаёт быть маршрутом и становится способом не торопить разговор.
...
* [Спросить, что помогает {npc_name} не торопиться]
    # speaker:mc
...
=== sunday_date_park_settle ===
# speaker:none
Разговор начинается с простого: погода, дорога, смешная неловкость у входа, кто сколько кофе уже успел выпить. Но рядом с {npc_name} простые темы не кажутся пустыми.
...
Уходить из парка пока рано. {npc_name} уже идёт к тебе.
        - else:
Уходить из парка пока рано. Сначала стоит хотя бы написать {npc_name}, где вы разминулись.
        }
        }
    }
# return_to_scene
