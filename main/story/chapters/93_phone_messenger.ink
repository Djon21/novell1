...
{npc_gender == "female":
    # msg:add:mila:Я всё думаю о том, что ты сказал. Не знаю, правильно ли я сделала, что поверила. Но перестать уже не могу.
    # msg:read:mila
- else:
    # msg:add:artem:Я всё думаю о том, что ты сказала. Не знаю, правильно ли я сделал, что поверил. Но перестать уже не могу.
    # msg:read:artem
}
->->
...
{iteration_number > 3:
Очередное воскресенье. Очередное сообщение.
- else:
Третье воскресенье. Третье сообщение.
} Наверное, я должен устать. Но нет — скорее привык.
...
# speaker:mc
Она написала снова. Не потому что хочет надавить — потому что ей правда не всё равно.

* [«Извини. Я был не в себе. Давай встретимся.»]
    # msg:reply:mila:Извини. Я был не в себе. Давай встретимся.
    # msg:add:mila:Звучит интригующе. Хорошо, давай.
    ~ TRUST = TRUST + 1
    -> msg_mila_loop2_place_choice

* [«Всё сложно. Но встретиться надо.»]
    # msg:reply:mila:Всё сложно. Но встретиться надо.
    # msg:add:mila:Ты меня пугаешь. Но ладно, встретимся.
    ~ INSIGHT = INSIGHT + 1
    -> msg_mila_loop2_place_choice

* [«Ок. Давай просто встретимся.»]
    # msg:reply:mila:Ок. Давай просто встретимся.
    # msg:add:mila:Хорошо. Где?
    ~ SYNC = SYNC + 1
    -> msg_mila_loop2_place_choice
...
# speaker:mc
Он написал снова. Не потому что хочет надавить — потому что ему правда не всё равно.

* [«Извини. Я была не в себе. Давай встретимся.»]
    # msg:reply:artem:Извини. Я была не в себе. Давай встретимся.
    # msg:add:artem:Звучит интригующе. Хорошо, давай.
    ~ TRUST = TRUST + 1
    -> msg_artem_loop2_place_choice

* [«Всё сложно. Но встретиться надо.»]
    # msg:reply:artem:Всё сложно. Но встретиться надо.
    # msg:add:artem:Ты меня пугаешь. Но ладно, встретимся.
    ~ INSIGHT = INSIGHT + 1
    -> msg_artem_loop2_place_choice

* [«Ок. Давай просто встретимся.»]
    # msg:reply:artem:Ок. Давай просто встретимся.
    # msg:add:artem:Хорошо. Где?
    ~ SYNC = SYNC + 1
    -> msg_artem_loop2_place_choice
...
# speaker:mc
Я знаю, как {npc_name_gen} ответит. {npc_gender == "female": Она|Он} повторяется, но это уже не раздражает — успокаивает.
-> msg_npc_place_sent
...
На экране — обрывки. Будто система вывела внутренние логи не в тот канал.

[Кейс 017 — Статус: Ожидает]
[Кейс 017 — Статус: Просрочено]
[Кейс 017 — Статус: Закрыто автоматом]
[Код операции: 017 — Статус: финальный]

{iteration_number > 1:
Последняя строка появлялась и в прошлый раз. Она не меняется.
}

* [Прочитать ещё раз]
    # msg:read:unknown
    # speaker:none
    Строки те же. Система просто повторяет то, что уже записала.
    # return_to_scene
    -> DONE
