...
{date_place_park:
    Парк остаётся в голове не одной картинкой, а несколькими: вход, вода, сообщение в Messenger, поиски {npc_name_gen}, лавочка или аллея — всё то, что сначала казалось маршрутом, а потом стало встречей.
- else:
    Кафе остаётся в голове тёплым шумом: стойка, чашки, место в уголке, проход с зеркалом, паузы, которые перестали быть неловкими ровно тогда, когда их перестали заполнять.
}

{sunday_went_to_shop:
    Магазин после этого вспоминается странно отчётливо: полки, свет, маленькая покупка, выбор, который выглядел бытовым, но почему-то оказался частью дня.
- else:
    Смотровая вспоминается шире, чем сама площадка: перила, город, лавочка, воздух над рекой и разговор, который легче держался рядом с открытым пространством.
}

{sunday_gift_bought:
    {sunday_gift_right:
        И ещё — фонарик-брелок. Нелепый предмет, если смотреть на него как на подарок. Очень точный, если смотреть на него как на обещание не теряться.
    - else:
        Подарок тоже всплывает в памяти. Не идеальный, не тот самый, но принятый бережно — так, будто важнее был не предмет, а попытка прийти не с пустыми руками.
    }
- else:
    Ты ничего не покупал{mc_gender == "female":а|} перед встречей. И это тоже почему-то кажется честным: день держался не на предмете в кармане, а на том, что вы всё-таки дошли друг до друга.
}
...
# speaker:none
Сообщение остаётся на экране чуть дольше, чем нужно. Воскресенье наконец-то складывается в цельную форму: утро, подготовка, маршрут, встреча, второй стоп, обратная дорога, ключи у двери, свет в коридоре.
...
{date_place_park:
    После парка в квартире особенно слышно, как далеко осталась вода: здесь нет реки, только трубы в стенах и тихий электрический гул.
- else:
    После кафе квартира кажется менее людной, но не пустой: запах кофе как будто задержался где-то на рукаве, хотя это уже невозможно проверить.
}

{sunday_went_to_shop:
    После магазина в памяти остаётся белый свет полок — слишком обычный для того, чтобы значить что-то важное. И всё же именно такие вещи почему-то держатся дольше всего.
- else:
    После смотровой город за кухонным окном выглядит как уменьшенная версия того вида: не панорама, а несколько горящих прямоугольников, но смысл почти тот же.
}
