...
- `game_state.add_sms()` сам проставляет fallback `time`, поэтому превью чата в `phone_v2` не требует отдельного Ink-поля времени; если позже понадобится каноничное сюжетное время сообщений, это уже отдельное расширение runtime-контракта
