...
> Для AI-сценариста это **низкоуровневая справка**, не основной шаблон.
> Каноничный формат новых хотспотов для сценарных задач: recipes из
> `main/data/scenes/_shared.lua`, см. `docs/scenario_context/LOCATIONS_GUIDE.md`
> и `docs/scenario_context/TEMPLATES.md`. Verbose-форма ниже описывает runtime-
> структуру после раскрытия recipe и не должна предлагаться как обычный формат.
