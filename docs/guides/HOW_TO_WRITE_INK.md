...
| `hud:hint:phone` | `# hud:hint:phone` | Кнопка PHN в HUD начинает пульсировать (подсказка игроку, что нужно открыть телефон). |
| `hud:hint:bag` | `# hud:hint:bag` | То же для кнопки инвентаря BAG. |
| `hud:hint:phone:off` / `hud:hint:bag:off` | — | Снять пульсацию явно. |
| `hud:hint:reset` | `# hud:hint:reset` | Снять обе пульсации. |

> **Авто-снятие пульса:** когда игрок сам открывает телефон или инвентарь — пульс гасится автоматически. Используй `# hud:hint:phone` сразу после `# sms:add:` если важно чтобы игрок ответил на сообщение, или `# hud:hint:bag` после `# add_item:` для нового важного предмета.
