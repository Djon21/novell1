...
_Сгенерировано: 2026-05-16 21:13_
...
| `shop_cleaning_supplies_interact` | На крючках висят перчатки, щётки и совки. Внизу стоят швабры, слишком прямые и терпеливые для предметов, которые обычно  |
| `shop_counter_interact` | Касса стоит у выхода, как граница между “взял с полки” и “это теперь часть дня”. Терминал, сканер, пустое место за стойк |
| `shop_drinks_interact` | Холодильники гудят ровно и уверенно. Вода, сок, холодный чай — маленькие решения, которые не требуют объяснений. |
| `shop_household_goods_interact` | Бытовой отдел выглядит так, будто здесь продают не вещи, а обещание маленького контроля: пакеты, губки, лампочки, батаре |
...
| `shop_sign_interact` | Красная полоса над входом ничего не объясняет и не пытается понравиться. Просто сообщает: открыто, свет горит, можно зай |
| `shop_snacks_interact` | Центральный стеллаж предлагает всё, что человек обычно покупает не потому, что нужно, а потому что день длиннее, чем каз |
| `shop_window_interact` | Витрина работает как обещание: внутри светлее, чем снаружи, и всё разложено так, будто достаточно выбрать правильную пол |
| `sunday_shop_arrival` | - else: |
| `sunday_shop_arrival_pre_date` | Внутри магазин оказывается почти пустым: белый свет, ровные полки, холодильники с напитками у дальней стены, касса у вых |
| `sunday_shop_arrival_with_npc` | Магазин 24/7 выглядит слишком ярким для воскресенья: белый свет, ровные полки, холодильники у дальней стены, касса у вых |
| `sunday_shop_settle` | Вы выходите из магазина с пакетом, который почти ничего не весит. Вода, сладкое, случайная мелочь — всё это не меняет де |
| `sunday_shop_street_arrival` | Магазин стоит внизу жилого дома, слишком яркий для обычного воскресного маршрута. Стекло отражает улицу, но внутри всё р |
