...
- [ ] Подключить bridge `dm.get_effects()` в активный `v2` UI, чтобы реально заработали Ink-теги `# sfx`, `# shake`, `# pulse`.
- [ ] Исправить loop-индикацию в диалоге: `dialogue_v2.gui_script` сейчас показывает `петля #017` по хардкоду, а `ui_manager_v2` не прокидывает туда текущую итерацию из `meta_state`.
