...
- `ui_manager_v2` дополнительно поддерживает legacy-derived sync для старых флагов `has_mug` и `has_phone`, чтобы `mug/phone` не расходились с реальным state между сценами и сейвами

## Ограничения MVP

- максимум `12` уникальных предметов
- текущие рабочие verbs: `use`, `inspect`, `read`
- `combine` и `give` пока не входят в рабочий flow
