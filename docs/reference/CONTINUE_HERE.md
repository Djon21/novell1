...
- исправлен баг вечного диалога спальни (`# set_flag:` не парсился → флаг не ставился → цикл)
- исправлены `# add_item:` / `# remove_item:` в `apply_tags`
...
- **протестировать SMS-flow вручную**: взять телефон → открыть SMS → тапнуть переписку с Милой → убедиться что knot открывается, флаги ставятся, квест закрывается
