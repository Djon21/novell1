...
**ТЕКУЩИЙ СТАТУС (2026-04-20, ветка AVOS_S, коммит ab82f63):**

✅ **ПРОБЛЕМА РЕШЕНА**: модалки `inventory_v2`, `phone_v2`, `map_v2` теперь отображаются корректно.

**Что было исправлено (коммиты de3de5f → ab82f63):**

1. **Alpha inheritance fix (de3de5f)**: Контейнеры `grid`, `app_grid`, `verbs_panel`, `doss_verbs` изменены с `color.w=0.0` на `color.w=1.0` и `size=0×0`. Это предотвратило наследование нулевой прозрачности дочерними нодами.

2. **Absolute coordinates (68c0c31)**: Убраны вызовы `gui.set_parent` для динамических нод. Все слоты, плитки, кнопки теперь создаются с абсолютными экранными координатами (`parent_pos + local_offset`). Defold не рендерит детей у нод с `size=0×0`, поэтому динамические ноды должны быть top-level.

3. **Z-index fix (ab82f63)**: Понижен z-index backdrop с 0.5-0.6 до 0.4 во всех модалках. Backdrop перекрывал динамический контент (z=0.55-0.57). Новый порядок: backdrop (0.4) < динамика (0.55-0.57) < статика (0.51-0.53).

**Тестовые изменения (будут удалены позже):**
- Коммит a710bac: добавлены тестовые предметы (mug, phone, key) при старте игры
- Коммиты 9a8625b, ffb04c8: дебаг-логирование

Все 20 шагов v2-интеграции выполнены. Bootstrap переключён на `main_v2.collection`. Hotspots реализованы. Модалки работают.
