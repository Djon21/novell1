...
- [ ] **step1**: скопировать шрифты JetBrains Mono / Unbounded / Caveat / Manrope в `main/fonts/` (скачать с Google Fonts если нет) и создать `.font` файлы для каждого начертания, которое реально используется (JBM-Regular/Bold, Unbounded-Bold/Black, Caveat-Regular, Manrope-Regular/Medium). Формат по аналогии с `main/fonts/material_icons.font`.
