...
**ТЕКУЩИЙ СТАТУС (2026-04-20, ветка AVOS_S, коммит 0a3d1a1):**

✅ **ВСЕ ОСНОВНЫЕ ПРОБЛЕМЫ РЕШЕНЫ**: модалки работают, фоны обновляются, объекты появляются корректно.

**Что было исправлено в этой сессии:**

1. **Модалки (коммиты de3de5f → ab82f63)**:
   - Alpha inheritance fix: контейнеры `color.w=1.0`, `size=0×0`
   - Absolute coordinates: убран `gui.set_parent`, используются абсолютные координаты
   - Z-index fix: backdrop понижен до 0.4, динамика 0.55-0.57, статика 0.51-0.53
   - Результат: inventory_v2, phone_v2, map_v2 отображаются корректно

2. **Фон в exploration (коммит ced9100)**:
   - scene_controller.set_background теперь обновляет визуальный фон в dialogue
   - handle_dialogue_update проверяет exploration-режим ПЕРЕД установкой фона из Ink
   - Результат: при переходе между комнатами фон обновляется корректно

3. **Объекты сцены (коммит 0a3d1a1)**:
   - Удалены тестовые предметы (mug, phone, key) и флаг has_phone
   - Результат: телефон появляется на тумбочке после выпивания кофе

**Известные ограничения:**
- Приложения телефона (СМС, Задачи и т.д.) закрывают телефон и показывают диалог на основном экране, а не внутри телефона. Это временное решение, требует отдельной реализации экранов внутри телефона.

**Тестовые изменения (можно удалить позже):**
- Коммиты 9a8625b, ffb04c8: дебаг-логирование в модалках
- Коммит 280be29: дебаг-логирование в scene_controller

Все 20 шагов v2-интеграции выполнены. Bootstrap переключён на `main_v2.collection`. Hotspots реализованы. Модалки работают. Игра полностью функциональна.
