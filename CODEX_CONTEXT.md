...
Актуально на `2026-04-20`, коммит `8ad053a`.
...
### Миграция на v2 выглядит не полностью завершённой

`dialogue_manager_ink.lua` умеет генерировать больше команд, чем сейчас явно обрабатывает `ui_manager_v2.script`.

Особенно стоит перепроверять поддержку:
...
Если работа касается телефона, квестов, заметок или тегов Ink, сначала сверять актуальную обработку этих команд.
