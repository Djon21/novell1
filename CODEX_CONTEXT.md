...
Актуально на `2026-04-20`, коммит `a972da2`, ветка `AVOS_S`.
...
### Миграция на v2 завершена, но есть проблемы с модалками

Все 20 шагов v2-интеграции выполнены. Bootstrap переключён на `main_v2.collection`. Hotspots реализованы и подключены.

**Текущая проблема**: модалки `inventory_v2` и `phone_v2` открываются (backdrop затемнение видно), но содержимое не отображается. Проблема связана с динамически создаваемыми нодами через `gui.new_box_node` / `gui.new_text_node`.

Известные фиксы:
- У контейнеров `color.w = 1.0` (не 0.0), `size = 0×0`
- У динамических детей `inherit_alpha = false`
- В `choice_v2` динамические ноды создаются с абсолютными координатами (без `set_parent` к контейнерам)
